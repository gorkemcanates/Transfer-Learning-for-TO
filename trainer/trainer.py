...
class MainTrainer:
    ...
    def adjust_lr(self):
        for param in self.optimizer.param_groups:
            param['lr'] = self.hyperparams.FINETUNE_LEARNING_RATE


    def adjust_gradients(self):
        for param in self.model.enc1.parameters():
            param.requires_grad = True
        for param in self.model.enc2.parameters():
            param.requires_grad = True
        for param in self.model.enc3.parameters():
            param.requires_grad = True
        for param in self.model.enc4.parameters():
            param.requires_grad = True
        for param in self.model.enc5.parameters():
            param.requires_grad = True
